...
def parse_args():
    ...
    parser.add_argument("--k-grid", default="3,5,7,9,11", type=str)

    parser.add_argument("--metric-grid", default="cosine,euclidean,manhattan", type=str)

    parser.add_argument("--weight-grid", default="uniform,distance", type=str)
...
def parse_int_grid(grid_str):

    return [int(x.strip()) for x in grid_str.split(",")]

def parse_str_grid(grid_str):

    return [x.strip() for x in grid_str.split(",")]
...
def knn_grid_search(x_train, y_train, x_val, y_val, k_grid, metric_grid, weight_grid):

    print("\n==============================")

    print("Grid Search: Raw AlexNet Feature + KNN")
    ...
    for rank, r in enumerate(all_results[:5], start=1):
        ...
        print(
            ...
            f"{rank}. k={r['k']}, metric={r['metric']}, weights={r['weights']} | "
...
def main():
    ...
    metric_grid = parse_str_grid(args.metric_grid)

    weight_grid = parse_str_grid(args.weight_grid)
